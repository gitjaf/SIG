...
!function($){
  ...
  * ================================= */
  ...
  , matcher: function (item) {
      return ~getObj(item, this.key).toLowerCase().indexOf(this.query.toLowerCase())
    }
  ...
  , sorter: function (items) {
      ...
      while (item = items.shift()) {
        if (!getObj(item, this.key).toLowerCase().indexOf(this.query.toLowerCase())) beginswith.push(item)
        else if (~getObj(item, this.key).indexOf(this.query)) caseSensitive.push(item)
  ...
  , highlighter: function (item, that) {
      ...
      return that.displayer(that, item, getObj(item, that.key).replace(new RegExp('(' + query + ')', 'ig'), function ($1, match) {
  ...
  , render: function (items) {
      ...
      items = $(items).map(function (i, item) {
        i = $(that.options.item).attr('data-key', getObj(item, that.key))
        i.attr('data-value', getObj(item, that.value))
  ...
  * ================== */
  ...
  /*
    FIX BY JFARINA
    ===============
    Las siguientes 2 funciones permiten pasar como key un objeto anidado, o un array de objetos anidados.
    En este ultimo caso la seleccion sera la concatenacion separadas por espacios de cada elemento
    dentro del array,

  */

  function getObj(element, key) {
    if($.isArray(key)){
      return getObjFromArray(element, key);
    }
    var i = key.indexOf('.');
    if(i > -1){
      return getObj(element[key.substring(0, i)],key.substring(i+1));
    } else{
      return (element[key]);
    }
  }

  function getObjFromArray(element, keys){
    var e = "";
    for(var k in keys){
      e = e + getObj(element, keys[k]) + " ";
    }
    return e;
  }
